...
import { ScheduleAvailabilityTable } from "@/app/drizzle/schema"
...
const groupedAvailabilities = schedule.availabilities.reduce((acc, availability) => {
    ...
    acc.get(day).push(availability);
    return acc;
  }, new Map());
  
  // If you still need it as an object, convert it back:
  const groupedAsObject = Object.fromEntries(groupedAvailabilities);
